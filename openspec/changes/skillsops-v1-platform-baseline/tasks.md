...
- [ ] 5.5 **前端**：安装面板：生成 UUID **Idempotency-Key** → 调接口 → 写剪贴板 → Toast（提示勿分享，语义与短时签名一致）；`/workspace/installed` 列表与复制最新命令
...
- [ ] 6.1 **后端**：Flyway `rating` 表、`uk_user_skill`；`PUT /v1/skills/{id}/ratings`；**未安装** `422 RATING_REQUIRES_INSTALL`；聚合 `avg_rating`/`rating_count` 与缓存失效
